...
def binomial_series(n, x, t):
    ...
    for r in range(1, t):
        ...
        coefficient = Decimal(coefficient) * ((Decimal('-1') * (Decimal(x) ** 2)) ** r)
...
def compute_pi(no_of_terms):
    series_coefficient = binomial_series(Decimal('0.5'), Decimal('0.5'), no_of_terms)
    new_series = []
    sum = Decimal('0')

    for i in range(no_of_terms):
        value = Decimal(series_coefficient[i] * Decimal(integration(Decimal(i))))
